...
/datum/action/innate/clockwork_armaments/Activate()
	...
	if(remove_item_if_better(I, owner))
		do_message += owner.equip_to_slot_or_del(new/obj/item/clothing/suit/armor/clockwork(null), ITEM_SLOT_ICLOTHING)
	...
	if(remove_item_if_better(I, owner))
		do_message += owner.equip_to_slot_or_del(new/obj/item/clothing/shoes/clockwork(null),
		ITEM_SLOT_FEET)
	if(do_message)
